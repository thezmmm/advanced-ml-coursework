...
def irls(X, labels, w, itercnt):
    ...
    def compute_stats(w):
        # Negative log-likelihood: sum_n log(1 + exp(-y_n * w^T x_n))
        nll = np.sum(np.log1p(np.exp(-y * (X @ w))))
        # Misclassification: predict 1 if w^T x >= 0, else -1
        preds = np.where(X @ w >= 0, 1.0, -1.0)
        mc = np.mean(preds != y)
        return nll, mc

    # Record stats for initial w
    err[0], misclass[0] = compute_stats(w)

    for t in range(1, itercnt + 1):
        # mu_n = sigmoid(w^T x_n)
        mu = sigmoid(X @ w)                          # (n,)

        # S_nn = mu_n * (1 - mu_n)
        s = mu * (1 - mu)                            # (n,)

        # Gradient: g = X^T (mu - (y + 1) / 2)
        g = X.T @ (mu - (y + 1) / 2)                # (k,)
        ...
        # Solve H delta = g instead of explicit inversion
        delta = np.linalg.solve(H + 1e-10 * np.eye(H.shape[0]), g)
        w = w - delta

        err[t], misclass[t] = compute_stats(w)
